...
const ENTRIES: ApiDocEntry[] = [
  ...
  },
  {
    ...
    title: "设备列表", summary: "列出全部设备（管道机器人/无人机）。", requiresAuth: true,
  ...
  },
  {
    ...
    title: "注册设备（待审批）", summary: "设备端发起注册申请，进入待审批状态；批准前无法上报数据（不签发凭据）。无需登录。", requiresAuth: false,
  ...
  },
  {
    ...
    title: "审批同意并签发密钥", summary: "管理端同意入网，一次性返回设备密钥（仅此一次，请立即配置到设备）。", requiresAuth: true,
  ...
  },
  {
    ...
    title: "拒绝入网", summary: "管理端拒绝设备注册申请。", requiresAuth: true,
    response: { ok: true },
  },
  {
    id: "devices-update", group: "设备与基站", method: "PATCH", path: `${P}/devices/:id`,
    title: "编辑设备", summary: "更新设备名称/类型/状态/描述。", requiresAuth: true,
    body: { status: "busy" },
    response: { id: 1, name: "机器人A", type: "crawler", status: "busy" },
  },
  {
    id: "devices-remove", group: "设备与基站", method: "DELETE", path: `${P}/devices/:id`,
    title: "删除设备", summary: "删除设备及其巡航路线。", requiresAuth: true,
    response: { ok: true },
  },
  {
    id: "devices-registration", group: "设备与基站", method: "GET", path: `${P}/devices/:id/registration`,
    title: "查询注册审批状态", summary: "设备端轮询审批状态（待审批/已注册/已拒绝/已注销），不返回密钥。无需登录。", requiresAuth: false,
    response: { id: 1, state: "pending", registeredAt: null },
  },
  {
    id: "devices-route-get", group: "设备与基站", method: "GET", path: `${P}/devices/:id/route`,
    title: "查询巡航路线", summary: "查询某设备基于数据源规划的巡航路线。无需登录（供设备端拉取）。", requiresAuth: false,
  ...
  },
  {
    ...
    title: "保存巡航路线", summary: "保存基于当前 GIS 数据源规划的巡航路线（经纬度点列）。", requiresAuth: true,
  ...
  {
    id: "devices-geo", group: "设备与基站", method: "GET", path: `${P}/devices/geo/:fileId`,
    title: "设备拉取 GIS 地图数据", summary: "已批准设备携带密钥 Header: x-device-key 与 ?device=设备ID 直连拉取数据源 GeoJSON，无需登录。", requiresAuth: false,
    query: { device: "设备 ID（必填）", "x-device-key": "设备密钥 Header（必填）" },
    response: { type: "FeatureCollection", features: [] },
  },
  {
    id: "devices-telemetry-push", group: "设备与基站", method: "POST", path: `${P}/devices/:id/telemetry`,
    title: "上报设备遥测", summary: "设备携带批准签发的密钥（Header: x-device-key）上报实时位置，校验后经 Socket 广播 `pm:telemetry`。无需登录。", requiresAuth: false,
  ...
  },
  {
    ...
    title: "设备最新位置", summary: "获取全部设备的最新遥测位置（供首页地图叠加，需登录）。", requiresAuth: true,
    response: { devices: [{ deviceId: 1, lon: 126.9, lat: 46.61, ts: 1 }] },
  },
  {
    id: "stations-list", group: "设备与基站", method: "GET", path: `${P}/base-stations`,
    title: "基站列表", summary: "列出地图上规划的基站（充电/中继/指挥）。无需登录（供设备端/地图展示）。", requiresAuth: false,
  ...
  },
  {
    ...
    title: "新增基站", summary: "在指定经纬度规划基站。", requiresAuth: true,
  ...
  },
  {
    ...
    title: "编辑基站", summary: "更新基站位置/用途/说明。", requiresAuth: true,
  ...
  },
  {
    ...
    title: "删除基站", summary: "删除基站。", requiresAuth: true,
    response: { ok: true },
  },
  {
