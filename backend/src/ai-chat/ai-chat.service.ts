...
const REDO_MAX_CORRECTION_PUSHES = 2;
...
function looksLikeRedoTaskRequest(content: string): boolean {
  ...
  const domain =
    /文件|数据|通知|已读|未读|消息|设置|密钥|AI|连接|会话|设备|登录|用户|密码|用户名|管道|管段|管网|图纸|底图|定位|搜索|查询|列表|清单|共享|删除|标记|检测|测试|运行|状态|内容|信息/i.test(
      content,
    );
  return redo && domain;
}
...
function hasToolableDomain(content: string): boolean {
  return /文件|数据|通知|已读|未读|消息|设置|密钥|AI|连接|会话|设备|登录|用户|密码|用户名|管道|管段|管网|图纸|底图|定位|搜索|查询|列表|清单|共享|删除|标记|检测|测试|运行|状态|内容|信息/i.test(
    content,
  );
}
      ...
      async () => {
        ...
        for (; round < MAX_LOOP_ROUNDS; round += 1) {
          ...
          if (toolCalls.length === 0) {
            ...
            // 通用“重复执行同一任务”纠正：用户再次要求同一操作，模型却未调用任何工具、
            // 只凭历史话术声称已经做过/结果相同/又刷新了一遍，则要求其重新调用对应工具
            const strongRedoAsk = looksLikeRedoTaskRequest(lastUserContent);
            const redoAsked =
              strongRedoAsk ||
              isRepeatOfEarlierUserRequest(lastUserContent, messages) ||
